...
var require_scanner = __commonJS({
  "node_modules/@emmetio/scanner/scanner.cjs"(exports2) {
    "use strict";
    ...
    function isAlphaNumeric(code2) {
      return isNumber(code2) || isAlpha(code2);
    }
    function isAlphaNumericWord2(code2) {
      return isNumber(code2) || isAlphaWord2(code2);
    }
    function isAlphaWord2(code2) {
      return code2 === 95 || isAlpha(code2);
    }
    function isUmlaut2(code2) {
      return code2 === 196 || code2 == 214 || code2 === 220 || code2 === 228 || code2 === 246 || code2 === 252;
    }
    function isWhiteSpace(code2) {
      return code2 === 32 || code2 === 9 || code2 === 160;
    }
    function isSpace4(code2) {
    ...
    function eatQuoted(stream, options) {
      ...
      const quote2 = stream.peek();
      if (stream.eat(isQuote3)) {
        while (!stream.eof()) {
          switch (stream.next()) {
            case quote2:
              stream.start = start;
    ...
    exports2.ScannerError = ScannerError2;
    exports2.default = Scanner2;
    exports2.eatPair = eatPair;
    exports2.eatQuoted = eatQuoted;
    exports2.isAlpha = isAlpha;
    exports2.isAlphaNumeric = isAlphaNumeric;
    exports2.isAlphaNumericWord = isAlphaNumericWord2;
    exports2.isAlphaWord = isAlphaWord2;
    exports2.isNumber = isNumber;
    exports2.isQuote = isQuote3;
    exports2.isSpace = isSpace4;
    exports2.isUmlaut = isUmlaut2;
    exports2.isWhiteSpace = isWhiteSpace;
  }
});

// node_modules/@emmetio/math-expression/dist/math.cjs.js
var require_math_cjs = __commonJS({
  "node_modules/@emmetio/math-expression/dist/math.cjs.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
    function _interopDefault(ex) {
      return ex && typeof ex === "object" && "default" in ex ? ex["default"] : ex;
    }
    var Scanner2 = require_scanner();
    var Scanner__default = _interopDefault(Scanner2);
    ...
    function parse2(expr) {
      ...
      const tokens = [];
      while (!scanner.eof()) {
        scanner.eatWhile(Scanner2.isWhiteSpace);
        scanner.start = scanner.pos;
        if (consumeNumber2(scanner)) {
          if ((expected & 1) === 0) {
            error2("Unexpected number", scanner);
          }
          tokens.push(number(scanner.current()));
          expected = 2 | 8;
        } else if (isOperator2(scanner.peek())) {
          ch = scanner.next();
          if (isSign(ch) && expected & 16) {
            ...
            if ((expected & 2) === 0) {
              error2("Unexpected operator", scanner);
            }
          ...
          error2("Unknown character", scanner);
        }
      }
      if (priority < 0 || priority >= 10) {
        error2('Unmatched "()"', scanner);
      }
      const result = orderTokens(tokens);
      if (result === null) {
        error2("Parity", scanner);
      }
      return result;
    }
    function consumeNumber2(scanner) {
      const start = scanner.pos;
      if (scanner.eat(
        46
        /* Dot */
      ) && scanner.eatWhile(Scanner2.isNumber)) {
        return true;
      }
      if (scanner.eatWhile(Scanner2.isNumber) && (!scanner.eat(
        46
        /* Dot */
      ) || scanner.eatWhile(Scanner2.isNumber))) {
        return true;
    ...
    function extract2(text2, pos = text2.length, options) {
      ...
      if (opt2.lookAhead && cur(scanner) === 41) {
        ...
        while (scanner.pos < len) {
          ch = cur(scanner);
          if (ch !== 41 && !(opt2.whitespace && Scanner2.isSpace(ch))) {
            break;
          }
          scanner.pos++;
        }
      ...
      while (scanner.pos >= 0) {
        ...
        if (ch === 41) {
          ...
          braces--;
        } else if (!(opt2.whitespace && Scanner2.isSpace(ch) || isSign(ch) || isOperator2(ch))) {
          break;
        }
        scanner.pos--;
      }
      if (scanner.pos !== end && !braces) {
        while (Scanner2.isSpace(cur(scanner))) {
          scanner.pos++;
        }
    ...
    function number$1(scanner) {
      if (Scanner2.isNumber(prev(scanner))) {
        scanner.pos--;
        let dot = false;
        let ch;
        while (scanner.pos >= 0) {
...
function findTagAt(text2, pos) {
  const insideTag = findOpenTagAt(text2, pos);
  if (insideTag) {
    return buildTagLocation(text2, insideTag);
  }
  return findEnclosingTag(text2, pos);
}
function findOpenTagAt(text2, pos) {
  let lt = pos;
  while (lt >= 0 && text2[lt] !== "<") {
    lt--;
  }
  if (lt < 0) return;
  if (text2[lt + 1] === "/") return;
  let gt = pos;
  while (gt < text2.length && text2[gt] !== ">") {
    if (text2[gt] === "<" && gt !== lt) return;
    gt++;
  }
  if (gt >= text2.length) return;
  const tagStr = text2.slice(lt + 1, gt);
  const nameMatch = tagStr.match(/^([a-zA-Z][a-zA-Z0-9:.-]*)/);
  if (!nameMatch) return;
  const name = nameMatch[1];
  const selfClose2 = tagStr.endsWith("/");
  const attrs = parseTagAttributes(text2, lt);
  return { name, from: lt, to: gt + 1, selfClose: selfClose2, attrs };
}
function findEnclosingTag(text2, pos) {
  let i = pos;
  while (i >= 0) {
    if (text2[i] === "<") {
      if (text2[i + 1] === "/") {
      ...
      const nameMatch = text2.slice(i + 1).match(/^([a-zA-Z][a-zA-Z0-9:.-]*)/);
      if (nameMatch) {
        const name = nameMatch[1];
        const gtIdx = findTagClose(text2, i);
        if (gtIdx === -1) {
          i--;
          continue;
        }
        const selfClose2 = text2.slice(i + 1, gtIdx).endsWith("/");
        if (selfClose2) {
          i--;
          continue;
        }
        const closeTag = findCloseTag(text2, name, gtIdx);
        if (closeTag && (gtIdx <= pos || closeTag.from >= pos)) {
          const attrs = parseTagAttributes(text2, i);
          return {
            name,
...
function buildTagLocation(text2, info) {
  if (info.selfClose) {
    return {
      name: info.name,
  ...
  const close = findCloseTag(text2, info.name, info.to);
  return {
    name: info.name,
    open: { from: info.from, to: info.to },
    close,
...
function findTagClose(text2, ltPos) {
  let i = ltPos + 1;
  let inQuote = null;
...
function findCloseTag(text2, name, startPos) {
  ...
  while (i < text2.length) {
    openRe.lastIndex = i;
    closeRe.lastIndex = i;
    const openMatch = openRe.exec(text2);
    const closeMatch = closeRe.exec(text2);
    if (!closeMatch) return;
    if (openMatch && openMatch.index < closeMatch.index) {
      const tagEnd = findTagClose(text2, openMatch.index);
      if (tagEnd !== -1 && !text2.slice(openMatch.index + 1, tagEnd).endsWith("/")) {
        depth++;
      }
...
function parseTagAttributes(text2, ltPos) {
  const gtPos = findTagClose(text2, ltPos);
  if (gtPos === -1) return [];
  const tagContent = text2.slice(ltPos + 1, gtPos);
  const attrs = [];
  ...
  while (pos < tagContent.length) {
    while (pos < tagContent.length && /\s/.test(tagContent[pos])) {
      pos++;
    }
    ...
    const nameRange = { from: base + attrNameStart, to: base + pos };
    while (pos < tagContent.length && tagContent[pos] === " ") {
      pos++;
    }
    if (tagContent[pos] !== "=") {
    ...
    const quote2 = tagContent[pos];
    if (quote2 === '"' || quote2 === "'") {
      const valueStart = pos + 1;
      pos++;
      while (pos < tagContent.length && tagContent[pos] !== quote2) {
        pos++;
      }
      const valueEnd = pos;
      pos++;
      const fullRange = { from: base + attrNameStart, to: base + pos };
      const valueRange = { from: base + valueStart, to: base + valueEnd };
      ...
      const fullRange = { from: base + attrNameStart, to: base + pos };
      const valueRange = { from: base + valueStart, to: base + pos };
      attrs.push({ full: fullRange, name: nameRange, value: valueRange });
    }
  }
  return attrs;
...
// src/lib/syntax.ts
var markupSyntaxes = /* @__PURE__ */ new Set([
  "html" /* html */,
  "xml" /* xml */,
  "xsl" /* xsl */,
  "jsx" /* jsx */,
...
var stylesheetSyntaxes = /* @__PURE__ */ new Set([
  "css" /* css */,
  "scss" /* scss */,
  "less" /* less */,
  "sass" /* sass */,
...
function getLanguageSyntax(languageId) {
  const map = {
    html: "html" /* html */,
    xml: "xml" /* xml */,
    xsl: "xsl" /* xsl */,
...
function lineIndentAt(doc, offset) {
  const line = lineAt(doc, offset);
  const m = line.text.match(/^(\s*)/);
  return m ? m[1] : "";
}
function detectIndent(editor) {
  return editor.options.insertSpaces ? " ".repeat(editor.options.tabSize) : "	";
}
function isQuote(ch) {
  return ch === '"' || ch === "'";
}
...
function narrowToNonSpace(text2, range) {
  const str = text2.slice(range.from, range.to);
  let start = 0;
  let end = str.length;
...
// src/commands/balance.ts
function balanceOutward(editor) {
  ...
  const nextSelections = editor.selections.map((sel) => {
    ...
    const ranges = getOutwardRanges(text2, offset);
    if (!ranges.length) return sel;
    found = true;
    const selRange = { from: doc.offsetAt(sel.start), to: doc.offsetAt(sel.end) };
    const target = ranges.find((r) => rangeContains(r, selRange) && !rangesEqual(r, selRange)) ?? ranges[0];
    return new vscode2.Selection(doc.positionAt(target.from), doc.positionAt(target.to));
  });
  if (found) {
    editor.selections = nextSelections;
  }
}
function balanceInward(editor) {
  if (!isHTML(editor.document.languageId)) return;
  const doc = editor.document;
  const text2 = doc.getText();
  let found = false;
  const nextSelections = editor.selections.map((sel) => {
    const offset = doc.offsetAt(sel.active);
    const ranges = getInwardRanges(text2, offset);
    if (!ranges.length) return sel;
    found = true;
    const selRange = { from: doc.offsetAt(sel.start), to: doc.offsetAt(sel.end) };
...
function getOutwardRanges(text2, pos) {
  const result = [];
  let searchPos = pos;
  for (let depth = 0; depth < 32; depth++) {
    const tag = findTagAt(text2, searchPos);
    if (!tag) break;
    const { open, close } = tag;
    if (close) {
      const inner = { from: open.to, to: close.from };
      const outer = { from: open.from, to: close.to };
...
function getInwardRanges(text2, pos) {
  const result = [];
  const tag = findTagAt(text2, pos);
  if (!tag || !tag.close) return result;
  const { open, close } = tag;
  result.push({ from: open.from, to: close.to });
  result.push({ from: open.to, to: close.from });
  const child = findTagAt(text2, open.to + 1);
...
function compactRanges(ranges, inward) {
  const sorted = [...ranges].sort(
    inward ? (a, b) => a.from - b.from || b.to - a.to : (a, b) => b.from - a.from || a.to - b.to
  );
  const result = [];
...
var import_math_expression = __toESM(require_math_cjs());
function evaluateMath(editor) {
  const doc = editor.document;
  const edits = [];
  for (const sel of editor.selections) {
    ...
    if (startOffset === endOffset) {
      ...
      const expr = (0, import_math_expression.extract)(line.text, sel.start.character);
      if (expr) {
        startOffset = doc.offsetAt(new vscode3.Position(sel.start.line, expr[0]));
        endOffset = doc.offsetAt(new vscode3.Position(sel.start.line, expr[1]));
      }
    }
    if (startOffset !== endOffset) {
      const text2 = doc.getText(new vscode3.Range(doc.positionAt(startOffset), doc.positionAt(endOffset)));
      try {
        const result = (0, import_math_expression.default)(text2);
        if (result !== null) {
          const insert = result.toFixed(4).replace(/\.?0+$/, "");
          edits.push({
            range: new vscode3.Range(doc.positionAt(startOffset), doc.positionAt(endOffset)),
            newText: insert
          });
        }
      } catch {
...
function goToNextEditPoint(editor) {
  moveToEditPoint(editor, 1);
}
function goToPreviousEditPoint(editor) {
  moveToEditPoint(editor, -1);
}
function moveToEditPoint(editor, inc) {
  const doc = editor.document;
  const text2 = doc.getText();
  editor.selections = editor.selections.map((sel) => {
    const offset = doc.offsetAt(sel.active);
    const next2 = findNewEditPoint(text2, offset + inc, inc);
    if (next2 != null) {
      const pos = doc.positionAt(next2);
      return new vscode4.Selection(pos, pos);
    }
    return sel;
  });
}
function findNewEditPoint(text2, pos, inc) {
  const docSize = text2.length;
  let cur = pos;
  while (cur < docSize && cur >= 0) {
    cur += inc;
    const ch = text2[cur];
    const next2 = text2[cur + 1];
    const prev = text2[cur - 1];
    if (isQuote(ch) && next2 === ch && prev === "=") {
      return cur + 1;
    }
    if (ch === "<" && prev === ">") {
      return cur;
    ...
    if (isNewLine(ch)) {
      const lineStart = findLineStart(text2, cur + Math.max(inc, 0));
      const lineEnd = findLineEnd(text2, lineStart);
      const lineText = text2.slice(lineStart, lineEnd);
...
function findLineStart(text2, pos) {
  let i = pos;
  while (i > 0 && text2[i - 1] !== "\n") {
    i--;
  }
...
function findLineEnd(text2, lineStart) {
  let i = lineStart;
  while (i < text2.length && text2[i] !== "\r" && text2[i] !== "\n") {
    i++;
  }
  return i;
}
...
function goToTagPair(editor) {
  ...
  const nextSelections = editor.selections.map((sel) => {
    const offset = doc.offsetAt(sel.active);
    const tag = findTagAt(text2, offset);
    if (tag && tag.close) {
      found = true;
      const { open, close } = tag;
      ...
      return new vscode5.Selection(pos, pos);
    }
    return sel;
  });
  if (found) {
...
var incrementNumber1 = (e) => incDecNumber(e, 1);
var decrementNumber1 = (e) => incDecNumber(e, -1);
var incrementNumber01 = (e) => incDecNumber(e, 0.1);
var decrementNumber01 = (e) => incDecNumber(e, -0.1);
var incrementNumber10 = (e) => incDecNumber(e, 10);
var decrementNumber10 = (e) => incDecNumber(e, -10);
function incDecNumber(editor, delta) {
  const doc = editor.document;
  const newSelections = [];
  editor.edit((eb) => {
    for (const sel of editor.selections) {
      ...
      if (startOffset === endOffset) {
        ...
        if (numRange) {
          startOffset = doc.offsetAt(new vscode6.Position(sel.start.line, numRange[0]));
          endOffset = doc.offsetAt(new vscode6.Position(sel.start.line, numRange[1]));
        }
      }
      if (startOffset !== endOffset) {
        const text2 = doc.getText(new vscode6.Range(doc.positionAt(startOffset), doc.positionAt(endOffset)));
        const newText = updateNumber(text2, delta);
        eb.replace(new vscode6.Range(doc.positionAt(startOffset), doc.positionAt(endOffset)), newText);
        const newEndPos = doc.positionAt(startOffset + newText.length);
        newSelections.push(new vscode6.Selection(newEndPos, newEndPos));
      } else {
...
function extractNumber(text2, pos) {
  ...
  while (start >= 0) {
    const ch = text2.charCodeAt(start - 1);
    if (isDot(ch)) {
      if (hasDot) break;
      hasDot = true;
    } else if (!isDigit(ch)) {
...
function removeTag(editor) {
  ...
  for (const sel of editor.selections) {
    ...
    const tag = findTagAt(text2, offset);
    if (!tag) continue;
    buildRemoveEdits(text2, doc, tag.open, tag.close, edits);
  }
  if (edits.length) {
...
function buildRemoveEdits(text2, doc, open, close, edits) {
  if (close) {
    const innerRange = narrowToNonSpace(text2, { from: open.to, to: close.from });
    if (!rangeEmpty(innerRange)) {
      edits.push({ range: new vscode7.Range(doc.positionAt(open.from), doc.positionAt(innerRange.from)), newText: "" });
      ...
      if (openLinePos.line !== closeLinePos.line) {
        ...
        const startLine = openLinePos.line + 2;
        ...
        for (let lineNum = startLine; lineNum <= endLine; lineNum++) {
          ...
          if (isSpace(lineText.slice(0, innerIndent.length))) {
            edits.push({
              range: new vscode7.Range(
                new vscode7.Position(lineNum, 0),
                new vscode7.Position(lineNum, innerIndent.length)
              ),
              newText: baseIndent
            });
          }
      ...
      edits.push({ range: new vscode7.Range(doc.positionAt(innerRange.to), doc.positionAt(close.to)), newText: "" });
    } else {
      edits.push({ range: new vscode7.Range(doc.positionAt(open.from), doc.positionAt(close.to)), newText: "" });
    }
  } else {
    edits.push({ range: new vscode7.Range(doc.positionAt(open.from), doc.positionAt(open.to)), newText: "" });
  }
}

// src/commands/select-item.ts
var vscode8 = __toESM(require("vscode"));
function selectNextItem(editor) {
  selectItem(editor, false);
}
function selectPreviousItem(editor) {
  selectItem(editor, true);
}
function selectItem(editor, reverse) {
  const doc = editor.document;
  const text2 = doc.getText();
  ...
  const nextSelections = editor.selections.map((sel) => {
    ...
    let range;
    if (isCSS(languageId)) {
      range = getCSSRange(text2, selRange, reverse);
    } else if (isHTML(languageId)) {
      range = getHTMLRange(text2, selRange, reverse);
    }
    if (range) {
      found = true;
      return new vscode8.Selection(doc.positionAt(range.from), doc.positionAt(range.to));
    }
    return sel;
  });
  if (found) {
...
function getHTMLRange(text2, sel, reverse) {
  const pos = reverse ? sel.from : sel.to;
  const tag = findTagAt(text2, pos);
  if (!tag) return;
  const candidates = getHTMLCandidates(text2, tag);
  return findRange(sel, candidates, reverse);
}
function getHTMLCandidates(text2, tag) {
  if (!tag) return [];
  ...
  for (const attr of attrs) {
    ...
    if (attr.value) {
      ...
      const attrName2 = text2.slice(attr.name.from, attr.name.to).toLowerCase();
      if (attrName2 === "class") {
        const valueStr = text2.slice(attr.value.from, attr.value.to);
...
function getCSSRange(text2, sel, reverse) {
  const pos = reverse ? sel.from : sel.to;
  const candidates = getCSSCandidatesAt(text2, pos);
  return findRange(sel, candidates, reverse);
}
function getCSSCandidatesAt(text2, pos) {
  const result = [];
  const declarationRe = /([a-zA-Z-]+)\s*:\s*([^;{}]+)/g;
  let m;
  while ((m = declarationRe.exec(text2)) !== null) {
    const propStart = m.index;
    const propEnd = propStart + m[0].length;
    if (propEnd < pos - 500 || propStart > pos + 500) continue;
    const nameRange = { from: propStart, to: propStart + m[1].length };
    const rawValue = m[2];
    const trimmedValue = rawValue.trimEnd();
    const valueFrom = propStart + m[0].indexOf(rawValue, m[1].length + 1);
    const valueRange = { from: valueFrom, to: valueFrom + trimmedValue.length };
    const fullRange = { from: propStart, to: propEnd };
...
function findRange(sel, ranges, reverse = false) {
  ...
  for (const r of ranges) {
    ...
    if (r.from === sel.from && r.to === sel.to) {
      ...
      if (rangeContains(r, sel) || (reverse ? r.from <= sel.from : r.from >= sel.from)) {
        candidate = r;
      }
    }
  }
  return needNext ? void 0 : candidate;
}
...
function splitJoinTag(editor) {
  ...
  for (const sel of editor.selections) {
    ...
    const { open, close } = tag;
    if (close) {
      const charBeforeGt = text2[open.to - 2];
      const closing = isSpace(charBeforeGt) ? "/" : " /";
      edits.push({
        range: new vscode9.Range(doc.positionAt(open.to - 1), doc.positionAt(close.to)),
        newText: `${closing}>`
      });
    } else if (tag.selfClose) {
      let insertFrom = open.to;
      let prefix = "";
      const inner = text2.slice(open.from + 1, open.to - 1);
      const slashIdx = inner.lastIndexOf("/");
      if (slashIdx !== -1) {
        let removeFrom = open.from + 1 + slashIdx;
        if (removeFrom > open.from + 1 && isSpace(text2[removeFrom - 1])) {
          removeFrom--;
        }
      ...
      edits.push({
        range: new vscode9.Range(doc.positionAt(insertFrom), doc.positionAt(open.to)),
...
function isAllowedSpace(ch, ctx) {
  return isSpace3(ch) && !ctx.expression;
}
...
function getExpandConfig(syntax, indent, text2) {
...
// src/commands/wrap-with-abbreviation.ts
async function wrapWithAbbreviation(editor) {
  const abbr = await vscode10.window.showInputBox({
  ...
  const snippets = [];
  for (const sel of editor.selections) {
    const selRange = sel.isEmpty ? doc.lineAt(sel.active.line).range : sel;
    const selText = doc.getText(selRange);
    const baseIndent = lineIndentAt(doc, doc.offsetAt(selRange.start));
    const lines = selText.split("\n").map(
      (line) => line.startsWith(baseIndent) ? line.slice(baseIndent.length) : line
    );
    const config = getExpandConfig(syntax, indent, lines);
    const expanded = expand(abbr, config);
    snippets.push({ selection: new vscode10.Selection(selRange.start, selRange.end), snippet: new vscode10.SnippetString(expanded) });
  }
...
function activate(context) {
  const reg = (id, fn) => context.subscriptions.push(
    vscode11.commands.registerTextEditorCommand(id, (editor) => fn(editor))
  );
  const regAsync = (id, fn) => context.subscriptions.push(
    vscode11.commands.registerTextEditorCommand(id, (editor) => fn(editor))
  );
